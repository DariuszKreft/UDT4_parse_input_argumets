...
void udt_parser::show_statistics(UDTSOCKET fhandle, UDT::TRACEINFO trace)
{
   ...
   cout << "After performance (m)-> UDT_MSS get value:" << block << endl;
   
   UDT::getsockopt(serv, 0, UDT_MAXBW, &block, &size);
   cout << "After performance (b)-> UDT_MAXBW get value:" << block << endl;
   
   UDT::getsockopt(fhandle, 0, UDT_RCVBUF, &block, &size_block);
   cout << "After performance (e)-> UDT_RCVBUF get value:" << block << endl;
   
   UDT::getsockopt(serv, 0, UDT_SNDBUF, &block, &size);
   cout << "After performance (o)-> UDT_RCVBUF get value:" << block << endl;
   
   UDT::getsockopt(serv, 0, UDP_SNDBUF, &block, &size);
   cout << "After performance (f)-> UDP_SNDBUF get value:" << block << endl;
   
   UDT::getsockopt(serv, 0, UDP_RCVBUF, &block, &size);
   cout << "After performance (t)-> UDP_RCVBUF get value:" << block << endl;
   
   UDT::getsockopt(serv, 0, UDT_SNDDATA, &block, &size);
   cout << "After performance (u)-> UDT_SNDDATA get value:" << block << endl;
   
   UDT::getsockopt(serv, 0, UDT_RCVDATA, &block, &size);
   cout << "After performance (i)-> UDT_RCVDATA get value:" << block << endl;
   
   UDT::getsockopt(serv, 0, UDT_FC, &block, &size);
   cout << "After performance (w)-> UDT_FC get value:" << block << endl;
   
   cout << "---------------------Performance global statistic---------------------\n";
   
   cout << "---------------------Performance local statistic---------------------\n";
   cout << performance_shows << endl;
   ...
   cout << "Busy sending time (i.e., idle time exclusive)" << trace.usSndDuration << endl
