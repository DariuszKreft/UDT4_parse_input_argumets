...
int main(int argc, char* argv[])
{
   ...
   int len = strlen(test->remote_file.c_str());
   ...
   if (UDT::ERROR == UDT::send(fhandle, test->remote_file.c_str(), len, 0))
   {
      cout << "send: " << UDT::getlasterror().getErrorMessage() << endl;
      return -1;
   }
   ...
   if (size < 0)
   {
      cout << "no such file " << test->remote_file << " on the server\n";
      return -1;
   }

   // receive the file
   fstream ofs(test->local_file.c_str(), ios::out | ios::binary | ios::trunc);
   int64_t recvsize; 
   int64_t offset = 0;
